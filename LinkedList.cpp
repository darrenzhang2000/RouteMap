//Darren Zhang 
//June 24, 2019
//Two methods are added to this file, one being a public (invert) and a private (invertRest)
//Invert is a wrapper to a recursive method that inverts the contents of the list
//invertRest is a helper function that does the inverting
...
// the item previously at position celing(item_count/2) is at position
 // ceiling(item_count_/2)
template<class T>
void LinkedList<T>::invert(){
	if(head_ptr_ == nullptr){ //base case, when list is empty 
		return; // does nothing
	}
	//inverts the rest 
	invertRest(head_ptr_);  //recursive step
}
...
void LinkedList<T>::invertRest(Node<T>* current_first_ptr){
	if(current_first_ptr -> getNext() == nullptr){ //base when: when pointer is at the end of the list
		head_ptr_ = current_first_ptr; 
		return;
	}
	invertRest(current_first_ptr -> getNext());
	current_first_ptr -> getNext() -> setNext(current_first_ptr); //creates reverse link 
	current_first_ptr -> setNext(nullptr); //disconnect forward link 
}
